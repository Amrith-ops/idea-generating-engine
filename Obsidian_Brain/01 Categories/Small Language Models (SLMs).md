...
# 📂 Category: Small Language Models (SLMs)

> G2 Software Category: Small Language Models (SLMs) within Artificial Intelligence.
...
FROM "02 Products"
WHERE contains(category, "Small Language Models (SLMs)") AND orbit_tier = "0_behemoth"
```
...
FROM "02 Products"
WHERE contains(category, "Small Language Models (SLMs)") AND orbit_tier = "1_challenger"
```
...
WHERE contains(category, "Small Language Models (SLMs)")
SORT osi_score DESC
```