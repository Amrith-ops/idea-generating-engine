...
> G2 Software Category: Key Value Databases within IT Infrastructure.
...
FROM "02 Products"
WHERE contains(category, "Key Value Databases") AND orbit_tier = "0_behemoth"
```
...
FROM "02 Products"
WHERE contains(category, "Key Value Databases") AND orbit_tier = "1_challenger"
```
...
WHERE contains(category, "Key Value Databases")
SORT osi_score DESC
```