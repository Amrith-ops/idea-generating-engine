...
> G2 Software Category: API Platforms within Development.
...
FROM "02 Products"
WHERE contains(category, "API Platforms") AND orbit_tier = "0_behemoth"
```
...
FROM "02 Products"
WHERE contains(category, "API Platforms") AND orbit_tier = "1_challenger"
```
...
WHERE contains(category, "API Platforms")
SORT osi_score DESC
```