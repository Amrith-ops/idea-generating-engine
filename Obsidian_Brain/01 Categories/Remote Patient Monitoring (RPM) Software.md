---
type: category
name: "Remote Patient Monitoring (RPM) Software"
slug: "remote-patient-monitoring-rpm"
---

# 📂 Category: Remote Patient Monitoring (RPM) Software

> G2 Software Category: Remote Patient Monitoring (RPM) Software within Vertical Industry.
...
FROM "02 Products"
WHERE contains(category, "Remote Patient Monitoring (RPM) Software") AND orbit_tier = "0_behemoth"
```
...
FROM "02 Products"
WHERE contains(category, "Remote Patient Monitoring (RPM) Software") AND orbit_tier = "1_challenger"
```
...
WHERE contains(category, "Remote Patient Monitoring (RPM) Software")
SORT osi_score DESC
```