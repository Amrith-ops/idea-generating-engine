...
# 📂 Category: Web Application Firewalls (WAF)

> G2 Software Category: Web Application Firewalls (WAF) within Security.
...
FROM "02 Products"
WHERE contains(category, "Web Application Firewalls (WAF)") AND orbit_tier = "0_behemoth"
```
...
FROM "02 Products"
WHERE contains(category, "Web Application Firewalls (WAF)") AND orbit_tier = "1_challenger"
```
...
WHERE contains(category, "Web Application Firewalls (WAF)")
SORT osi_score DESC
```