...
# 📂 Category: API Security Tools

> G2 Software Category: API Security Tools within Security.
...
FROM "02 Products"
WHERE contains(category, "API Security Tools") AND orbit_tier = "0_behemoth"
```
...
FROM "02 Products"
WHERE contains(category, "API Security Tools") AND orbit_tier = "1_challenger"
```
...
WHERE contains(category, "API Security Tools")
SORT osi_score DESC
```