...
> G2 Software Category: Time Series Databases within IT Infrastructure.
...
FROM "02 Products"
WHERE contains(category, "Time Series Databases") AND orbit_tier = "0_behemoth"
```
...
FROM "02 Products"
WHERE contains(category, "Time Series Databases") AND orbit_tier = "1_challenger"
```
...
WHERE contains(category, "Time Series Databases")
SORT osi_score DESC
```