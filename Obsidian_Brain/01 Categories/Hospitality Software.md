...
> G2 Software Category: Hospitality Software within Vertical Industry.
...
FROM "02 Products"
WHERE contains(category, "Hospitality Software") AND orbit_tier = "0_behemoth"
```
...
FROM "02 Products"
WHERE contains(category, "Hospitality Software") AND orbit_tier = "1_challenger"
```
...
WHERE contains(category, "Hospitality Software")
SORT osi_score DESC
```