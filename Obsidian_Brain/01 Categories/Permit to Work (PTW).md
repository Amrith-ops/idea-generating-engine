...
slug: "permit-to-work-ptw"
---

# 📂 Category: Permit to Work (PTW)

> G2 Software Category: Permit to Work (PTW) within ERP.
...
FROM "02 Products"
WHERE contains(category, "Permit to Work (PTW)") AND orbit_tier = "0_behemoth"
```
...
FROM "02 Products"
WHERE contains(category, "Permit to Work (PTW)") AND orbit_tier = "1_challenger"
```
...
WHERE contains(category, "Permit to Work (PTW)")
SORT osi_score DESC
```