...
> G2 Software Category: Applied Behavior Analysis (ABA) within Vertical Industry.
...
FROM "02 Products"
WHERE contains(category, "Applied Behavior Analysis (ABA)") AND orbit_tier = "0_behemoth"
```
...
FROM "02 Products"
WHERE contains(category, "Applied Behavior Analysis (ABA)") AND orbit_tier = "1_challenger"
```
...
WHERE contains(category, "Applied Behavior Analysis (ABA)")
SORT osi_score DESC
```