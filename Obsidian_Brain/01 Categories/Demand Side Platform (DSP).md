...
slug: "demand-side-platform-dsp"
---

# 📂 Category: Demand Side Platform (DSP)

> G2 Software Category: Demand Side Platform (DSP) within Digital Advertising Tech.
...
FROM "02 Products"
WHERE contains(category, "Demand Side Platform (DSP)") AND orbit_tier = "0_behemoth"
```
...
FROM "02 Products"
WHERE contains(category, "Demand Side Platform (DSP)") AND orbit_tier = "1_challenger"
```
...
WHERE contains(category, "Demand Side Platform (DSP)")
SORT osi_score DESC
```